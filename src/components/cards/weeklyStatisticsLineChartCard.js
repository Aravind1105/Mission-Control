  ...
  render() {
    return (
      <Card className={classnames(this.props.cardBgColor)}>
        <CardHeader>
          <CardTitle className="mb-0 white">{this.props.cardTitle}</CardTitle>
        </CardHeader>
        <CardBody className="pt-2 pb-0">
          ...
          <div className="my-3 text-center white">
            <Link to="/" className="font-large-2 d-block mb-1 white">
              {this.props.statisticsAmount} <span className="ft-arrow-up font-large-2" />
            </Link>
            <span className="font-medium-1">{this.props.statisticsRangeAmount}</span>
          </div>
        </CardBody>
        <ChartistGraph
