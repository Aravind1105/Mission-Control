...
const Utils = {
	...
	detectLanguage: (callback) => {
		...
		if(!window.l || typeof window.l==='undefined'){
			...
			window.l = (typeof navigator === 'undefined' ? window.defaultLanguage : ((navigator.languages && navigator.languages[0]) || navigator.language || navigator.userLanguage || window.defaultLanguage).toLowerCase() );
