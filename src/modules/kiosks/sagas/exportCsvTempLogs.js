...
import { exportCsvTempLogs as action } from '../actions';

export function handlerGetProduct(payload) {
  let link;
  link = `/api/v1/kiosk/csv/export/temperature/${payload.from}/${payload.to}?kioskId=${payload.kiosk}`;
    ...
    .then(response => {
      if (response.status != 200) {
        ...
        response.blob().then(blob => {
          ...
          element.href = url;
          element.download = `TempLog_${Date.now()}.csv`;
