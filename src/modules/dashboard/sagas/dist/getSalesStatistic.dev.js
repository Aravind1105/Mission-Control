...
function handler(_ref) {
  var _ref$payload, kioskId, time, variables, _ref2, data, products, queryName, selector, weekFormat, _getLastWeek, lastWeekDate, lastWeekFormat, formatted, res, dataArray;

  return regeneratorRuntime.wrap(function handler$(_context) {
    while (1) {
      switch (_context.prev = _context.next) {
        case 0:
          _ref$payload = _ref.payload, kioskId = _ref$payload.kioskId, time = _ref$payload.time;
          variables = kioskId ? {
            kioskId: kioskId
          } : {};
          _context.prev = 2;
        ...
        case 5:
          ...
          data[queryName].forEach(function (elem) {
            if (time === 'daily') {
              ...
              elem._id.date = _formatData.days[new Date(elem._id.date).getDay()];
            }
          });

          if (time === 'weekly') {
            _getLastWeek = getLastWeek(), lastWeekDate = _getLastWeek.lastWeekDate, lastWeekFormat = _getLastWeek.lastWeekFormat; // filter last 7 days including current date

            data[queryName] = data[queryName].filter(function (elem) {
              return lastWeekDate < new Date(elem._id.dateString);
            }); // sort the dates

            data[queryName] = data[queryName].sort(function (firstDate, secondDate) {
              return new Date(firstDate._id.dateString) - new Date(secondDate._id.dateString);
            }); // week day ordering 
          ...
          res = Object.keys(formatted).map(function (key) {
            var obj = formatted[key].reduce(function (prev, _ref3) {
              ...
              return _objectSpread({}, prev, _defineProperty({}, kiosk, total));
            }, {});
            return _objectSpread({
              name: key
            }, obj);
          });
          dataArray = (0, _formatData.formatData)(res, time, kioskId, weekFormat);
          _context.next = 17;
          return (0, _effects.put)((0, _actions.getSalesStatisticSuccess)({
            statistic: dataArray,
            products: products
          }));
