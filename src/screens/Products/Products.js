    ...
    return (
      <div>
        <div className="form-group form-group-compose text-left">
          ...
          <button type="button" className="btn btn-danger float-right my-2 shadow-z-2" onClick={this.toggle}>
        ...
        <div>
          <Table hover>
            ...
            <tbody>
              {productsList.hasOwnProperty('length') &&
                productsList.length > 0 &&
                productsList.map((object, i) => {
                  return (
                    <tr key={i}>
                      <td>
                        <img src={no_pic} className="media-object round-media height-50" alt="Card cap 02" />
                      </td>
                      <td>{object.name}</td>
                      ...
                      <td>
                        {Number(
                          (object.price * (object.productItems != null ? Number(object.productItems) : 1)).toFixed(2),
                        )}
                      </td>
                      <td>
                        <Edit size={18} className="mr-2" /> <Trash2 size={18} color="#FF586B" />
                      </td>
                    </tr>
                  );
                })}
